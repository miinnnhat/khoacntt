...
<form action="<?php echo $_CB_framework->viewUrl( 'logout', true, null, 'html', $secureForm ); ?>" method="post" id="login-form-<?php echo $module->id; ?>" class="mod-login-logout form-vertical cbLogoutForm">
	...
	<?php if ( $preLogoutText ) { ?>
		<div class="mod-login-logout__pretext pretext <?php echo htmlspecialchars( $templateClass ); ?>">
			<p><?php echo $preLogoutText; ?></p>
		</div>
	<?php } ?>
	<?php echo modCBLoginHelper::getPlugins( $params, $type, 'almostStart' ); ?>
	<?php if ( (int) $params->get( 'greeting', 1 ) ) { ?>
		<div class="mb-2 mod-login-logout__login-greeting login-greeting <?php echo htmlspecialchars( $templateClass ); ?>">
			<?php echo $greetingText; ?>
		</div>
	<?php } ?>
	<?php if ( (int) $params->get( 'show_avatar', 1 ) ) { ?>
		<div class="mb-2 mod-login-logout__login-avatar login-avatar <?php echo htmlspecialchars( $templateClass ); ?>">
	...
	<?php if ( $profileViewText || $profileEditText || $showPrivateMessages || $showConnectionRequests ) { ?>
		<ul class="mt-2 mb-0 mod-login-logout__options list-unstyled">
			<?php if ( $showPrivateMessages ) { ?>
				<li class="logout-options-private-messages">
			...
			<?php if ( $showConnectionRequests ) { ?>
				<li class="logout-options-connection-requests">
			...
			<?php if ( $profileViewText ) { ?>
				<li class="logout-options-profile">
			...
			<?php if ( $profileEditText ) { ?>
				<li class="logout-options-profile-edit">
	...
	<?php if ( $postLogoutText ) { ?>
		<div class="mod-login-logout__posttext posttext <?php echo htmlspecialchars( $templateClass ); ?>">
