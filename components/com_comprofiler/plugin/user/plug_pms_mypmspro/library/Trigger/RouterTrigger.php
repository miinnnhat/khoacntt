			...
			if ( isset( $query['func'] ) ) {
				...
				if ( $func == 'new' ) {
					if ( isset( $query['to'] ) ) {
						...
						if ( $to && is_numeric( $to ) ) {
							$username		=	\CBuser::getUserDataInstance( (int) $to )->getString( 'username', '' );
	...
	public function parse( $router, $plugin, $segments, &$vars, $menuItem )
	{
		...
		if ( isset( $segments[0] ) ) {
			if ( isset( $segments[1] ) ) {
				...
				if ( in_array( $action, array( 'edit', 'save', 'read', 'unread', 'delete', 'show', 'report', 'quick' ) ) ) {
					$vars['action']		=	'message';
					$func				=	$action;
					$id					=	$segments[1];
				...
				if ( is_numeric( $func ) ) {
					$vars['func']		=	'show';
					$vars['id']			=	$func;
				} elseif ( ! in_array( $func, array( 'received', 'sent', 'modal', 'quick', 'new', 'edit', 'save', 'read', 'unread', 'delete', 'show', 'report' ) ) ) {
					$vars['func']		=	'new';
					$vars['to']			=	$func;
				} else {
					$vars['func']		=	$func;
				...
				if ( in_array( $func, array( 'edit', 'save', 'read', 'unread', 'delete', 'report', 'quick' ) ) ) {
					$vars['action']		=	'message';
				} else {
					$vars['action']		=	'messages';
				}

				if ( in_array( $func, array( 'received', 'sent', 'modal', 'quick', 'new', 'edit', 'save', 'read', 'unread', 'delete', 'show', 'report' ) ) ) {
					$vars['func']		=	$func;
				} else {
					$vars['func']		=	'new';
					$vars['to']			=	$func;
