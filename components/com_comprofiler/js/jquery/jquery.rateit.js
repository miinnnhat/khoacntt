    ...
    $.fn.rateit = function (p1, p2) {
        ...
        return this.each(function () {
            ...
            if (isfont) {
                ...
                // Implemented to determine the size of a star even if it's a hidden element
                var cssFontSize = range.css( 'font-size' );
                var width = range.width();

                if ( width === 0 ) {
                    if ( ! fontSizeCache.hasOwnProperty( cssFontSize ) ) {
                        var cssWidth = range.css( 'width' );
                        var cssHeight = range.css( 'height' );

                        var temporary = range.clone( false ).attr({
                            id: ''
                        }).css({
                            position: 'absolute',
                            display: 'block',
                            ...
                            padding: range.css( 'padding' ),
                            border: range.css( 'border' ),
                            margin: range.css( 'margin' ),
                            fontFamily: range.css( 'font-family' ),
                            fontSize: cssFontSize,
                            fontWeight: range.css( 'font-weight' ),
                            lineHeight: range.css( 'line-height' ),
                            boxSizing: range.css( 'box-sizing' ),
                            wordSpacing: range.css( 'word-spacing' )
                        }).appendTo( 'body' );
                        ...
                        fontSizeCache[cssFontSize] = width;
                    } else {
                        width = fontSizeCache[cssFontSize];
                    }
                } else {
                    fontSizeCache[cssFontSize] = width;
                }

                itemdata('starwidth', width / (itemdata('max') - itemdata('min')))
            }
            else {
