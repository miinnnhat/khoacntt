...
$task				=	Application::Input()->getCmd( 'view', '' );
...
<?php
	if ( count( $_CB_Backend_Title ) > 0 ) {
		...
		if ( preg_match('/>([\w\s]+)</i', $pageTitle, $matches ) ) {
			...
			$cleanTitle			=	Get::clean( $pageTitle, GetterInterface::STRING );
		}
		...
		if ( isset( $_CB_Backend_Title[0][0] ) ) {
			...
			ToolbarHelper::title( $title, 'communitybuilder' );
		} else {
			ToolbarHelper::title( $cleanTitle );
		}

		$_CB_framework->setPageTitle( $_CB_framework->getCfg( 'sitename' ) . ' - ' . Text::_( 'JADMINISTRATION' ) . ' - ' . $cleanTitle );
	}
