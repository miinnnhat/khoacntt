...
class CBController_users {
	...
	public function showUsers( $option, $task, $cid ) {
		...
		if ( $task == 'resendconfirmationemails' ) {
			...
			$emailBody						=	Application::Input()->get( 'post/emailbody', '', ( Application::MyUser()->isSuperAdmin() ? GetterInterface::RAW : GetterInterface::HTML ) );
			...
			if ( $task == 'emailusers' ) {
				...
				if ( ! Application::Session()->checkFormToken( 'post', 0 ) ) {
					echo json_encode( [ 'result' => 0, 'htmlcontent' => CBTxt::Th( 'UE_SESSION_EXPIRED', 'The most recent request was denied because it had an invalid security token. Please go back or refresh the page and try again.' ) ] );
					return;
				}

				$cbNotification				=	new cbNotification( [ 'convertLinebreaks' => false ] );
				$mode						=	1; // html
				$errors						=	0;
				$success					=	array();
				$failed						=	array();
	...
	public function parseLinkURL( $matches ) {
		...
		if ( ( substr( $url, 0, 6 ) == 'mailto' ) || ( substr( $url, 0, 1 ) == '#' ) || ( substr( $url, 0, 4 ) == 'http' ) || ( substr( $url, 0, 1 ) == '[' ) ) {
