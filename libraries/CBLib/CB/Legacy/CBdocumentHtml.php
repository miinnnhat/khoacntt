...
class CBdocumentHtml
{
	...
	{
		if ( $getDocFunction ) {
			$this->_cmsDocFunc	=	$getDocFunction;
		}
	...
	/**
	 * Returns Joomla document object if available
	 *
	 * @return null|bool|JDocument
	 */
	protected function getDocument()
	{
		if ( ! $this->_cmsDocFunc ) {
			return $this->_cmsDocFunc;
		}

		if ( ! $this->_cmsDoc ) {
			$this->_cmsDoc	=	call_user_func_array( $this->_cmsDocFunc, [] );
		}
	...
	public function addHeadStyleSheet( $url, $minVersion = false, $media = null, $attributes = null, $type = 'text/css' )
	{
		...
		if ( $this->_tryCmsDoc( 'addStyleSheet', array( $url, $options, $attributes ) ) ) {
			return;
		}
...
//		}
	...
	public function getDirection( )
	{
		if ( $this->_direction === null ) {
			if ( $this->getDocument() ) {
				$this->_direction	=	$this->getDocument()->getDirection();
			} else {
				$this->_direction	=	'ltr';
			}
		}
	...
	{
		if ( $this->getDocument() ) {
			$this->getDocument()->setDirection( $textDirection );
		}
	...
	{
		if ( $this->getDocument() ) {
			call_user_func_array( [ $this->getDocument(), $type ], $params );

			return true;
		}

		if ( $this->getDocument() === false ) {
	...
	protected function _renderCheckOutput( )
	{
		...
		if ( $this->_headsOutputed && ( $this->getDocument() !== false ) ) {
//			$customHead			=	$this->_renderHead();
