...
class CBDatabaseChecker
{
	 ...
	 */
	public function checkDatabase( $upgrade = false, $dryRun = false, $strictlyColumns = null, $strictlyEngine = null, $strictlyCollation = null )
	{
		$xml							=	$this->_getCbDbXml();
		if ( $xml !== null ) {
			$db							=	$xml->getElementByPath( 'database' );
			if ( $db ) {
				$this->_sqlUpgrader		=	new DatabaseUpgrade( $this->_db, $this->_silentWhenOK );
				$this->_sqlUpgrader->setDryRun( $dryRun );
				$success				=	$this->_sqlUpgrader->checkXmlDatabaseDescription( $db, '', $upgrade, $strictlyColumns, $strictlyEngine, $strictlyCollation );
				/*
				var_dump( $success );
				echo "<br>\nERRORS: " . $this->_sqlUpgrader->getErrors( "<br /><br />\n\n", "<br />\n" );
				echo "<br>\nLOGS: " . $this->_sqlUpgrader->getLogs( "<br /><br />\n\n", "<br />\n" );
...
//			. "pf.name AS fieldpluginname, pf.published AS fieldpluginpublished "
		...
		try {
			...
			$this->_sqlUpgrader->setError( 'Fields selection query error: ' . $e->getMessage() );
			return false;
		}
	...
	protected function _checkIfCBMandatoryOK( )
	{
		...
		try {
			...
			$this->_sqlUpgrader->setError( 'Tabs selection query error: ' . $e->getMessage() );
			return false;
		}
	...
	public static function renderDatabaseResults( &$dbChecker, $upgrade, $dryRun, $result, $messagesBefore, $messagesAfter, $dbName, $dbId, $showConclusion = true ) {
		...
		if ( $messagesBefore ) {
			$return					.=		'<div class="mb-2 cbDbResultsMsgs">';
		...
		if ( $dbChecker !== null ) {
			$return					.=		'<div class="mb-2 cbDbResultsCheck">';
			...
			if ( $result == true ) {
				...
				if ( ! $upgrade ) {
					$return			.=			'<div class="mt-2 text-danger cbDbResultsErrorsFix">'
		...
		if ( $showConclusion ) {
			if ( $upgrade ) {
				if ( $dryRun ) {
					$return			.=		'<div class="mb-2 cbDbResultsConclusion">'
