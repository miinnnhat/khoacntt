...
class Image {
	...
	public function processImage( $maxWidth, $maxHeight, $maintainAspectRatio = null, $alwaysResample = null ) {
		...
		if ( ( $imageWidth > $maxWidth ) || ( $imageHeight > $maxHeight ) || $alwaysResample ) {
			...
			if ( ! $maintainAspectRatio ) {
				$image						=	$image->resize( $newImageSize )->save( $destination . $filename, ( $this->getCleanExt() == 'gif' ? array( 'animated' => true ) : array() ) );
			} else {
				...
				$image						=	$image->thumbnail( $newImageSize, $imageMode )->save( $destination . $filename, ( $this->getCleanExt() == 'gif' ? array( 'animated' => true ) : array() ) );
			}
		} else {
			$image							=	$image->save( $destination . $filename, ( $this->getCleanExt() == 'gif' ? array( 'animated' => true ) : array() ) );
		}
	...
	public function rotateImage( $angle, $background = null, $save = false ) {
		...
		if ( $background ) {
			$background		=	$image->palette()->color( $background );
		}
