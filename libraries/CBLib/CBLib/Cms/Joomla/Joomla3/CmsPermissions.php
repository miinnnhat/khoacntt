...
class CmsPermissions implements CmsPermissionsInterface
{
	...
	public function getAllGroups( $html = false, $indentString = '| ' )
	{
		...
		foreach ( $groups as $group ) {
			$indentedText		=	str_repeat( $indentString, $group->level ) . Text::_( $group->text );

			if ( $html ) {
				$userGroups[]	=	HTMLHelper::_( 'select.option', $group->value, $indentedText );
			} else {
	...
	public function getAllViewAccessLevels( $html = false, AuthoriseInterface $filterByVisibleToUser = null )
	{
		...
		foreach ( $levels as $value => $text ) {
			...
			if ( $html ) {
				$access_levels[]		=	HTMLHelper::_( 'select.option', $value, $text );
			} else {
	...
	public function getGroupName( $groupId, $indentString = null )
	{
		...
		if ( isset( $groups[$groupId] ) ) {
			$text				=	Text::_( $groups[$groupId]->text );
	...
	/**
	 * Clears Joomla access caches
	 *
	 * @return void
	 */
	public function clearStatics(): void
	{
		Access::clearStatics();
	}
}
