...
class DatabaseUpgrade
{
	 ...
	 */
	public function checkXmlDatabaseDescription( SimpleXMLElement $db, $colNamePrefix = '', $change = false, $strictlyColumns = null, $strictlyEngine = null, $strictlyCollation = null )
	{
		$isMatching								=	false;
	...
	public function checkXmlTableDescription( SimpleXMLElement $table, $colNamePrefix = '', $change = false, $strictlyColumns = false, $strictlyEngine = false, $strictlyCollation = false )
	{
		...
		if ( $table->getName() == 'table' ) {
			...
			if ( $tableName ) {
				...
				if ( $strictlyEngine !== false ) {
					$tableEngine							=	null;

					if ( $engine !== false ) {
						$tableType							=	$engine->attributes( 'type' );
						...
						if ( $tableType !== null ) {
							...
							if ( $sameEngine ) {
								...
								$tableEngine				=	$tableType;
							}
						}
					} elseif ( $strictlyEngine === null ) {
						// We were expecting to inherit but no engine node was supplied so force check against defaults:
						$strictlyEngine						=	true;
					}

					// Only compare engine if we're even allowed to change it:
					if ( ( $strictlyEngine === 'true' ) || ( $strictlyEngine === true ) || ( $strictlyEngine === 'updatewithtable' ) ) {
						if ( ! $tableEngine ) {
						...
						if ( $currentEngine !== $tableEngine ) {
							$this->setError( sprintf( 'Table %s Storage Engine is %s instead of %s', $tableName, $currentEngine, $tableEngine ) );

							if ( ( ! $change ) || ( ! $this->changeEngine( $tableName, $tableEngine ) ) ) {
								$isMatching					=	false;
							}
						}
					}
				...
				if ( $strictlyCollation !== false ) {
					...
					if ( $collation !== false ) {
						...
						if ( $collationType !== null ) {
							...
							if ( $collationTable !== null ) {
								$sameCollation				=	$this->checkTableCollation( $collationTable );
							} else {
								$sameCollation				=	null;
							}
					...
					} elseif ( $strictlyCollation === null ) {
						// We were expecting to inherit but no collation node was supplied so force check against defaults:
						$strictlyCollation					=	true;
					}

					// Only compare collation if we're even allowed to change it:
					if ( ( $strictlyCollation === 'true' ) || ( $strictlyCollation === true ) || ( $strictlyCollation === 'updatewithtable' ) ) {
						if ( ! $tableCollation ) {
							// Fallback to legacy attribute
							$tableCollation					=	$table->attributes( 'collation' );
						}

						if ( ! $tableCollation ) {
							$tableCollation					=	$this->defaultCollation;
						}

						$currentCollation					=	$this->checkTableCollation( $tableName );

						if ( $currentCollation !== $tableCollation ) {
							$this->setError( sprintf( 'Table %s Collation is %s instead of %s', $tableName, $currentCollation, $tableCollation ) );

							if ( ( ! $change ) || ( ! $this->changeCollation( $tableName, $tableCollation ) ) ) {
								$isMatching					=	false;
							}
						} elseif ( $allColumns ) {
							// Table itself is correct, but is its columns? If not then fix them!
							$mismatchColumns				=	[];

							foreach ( $allColumns as $column => $columnDetails ) {
								$columnCollation			=	( $columnDetails->Collation ?? '' );

								if ( ( ! $columnCollation ) || ( $columnCollation === $tableCollation ) ) {
									continue;
								}

								$mismatchColumns[$column]	=	$columnCollation;
								break;
							}

							if ( $mismatchColumns ) {
								foreach ( $mismatchColumns as $mismatchColumn => $mismatchColumnCollation ) {
									$this->setError( sprintf( 'Table %s Column %s Collation is %s instead of %s', $tableName, $mismatchColumn, $mismatchColumnCollation, $tableCollation ) );
								}
	...
	{
		$charSet				=	substr( (string) $tableCollation, 0, strpos( (string) $tableCollation, '_' ) );
		...
		if ( $table->getName() == 'table' ) {
			...
			if ( $tableName && ( $columns !== false ) ) {
				...
				if ( $charSet ) {
					...
					$tableOptions[]			=	'COLLATE = ' . preg_replace( '/[^a-z0-9_]/', '' , (string) $tableCollation );
				}
