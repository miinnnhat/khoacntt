...
class CBTxt
{
	 ...
	 * @param bool   $change   True (default): Changes the current language to the imported $language
	 * @return bool               True if language loaded successfully
	 */
	public static function import( $langPath, $language, $filename, $fallback = true, $override = false, $change = true )
	{
		if ( isset( static::$self->importedLangPathFiles[$language][$langPath][$filename] ) ) {
			return true;
		}

		static::$self->importedLangPathFiles[$language][$langPath][$filename]	=	false;
		...
		if ( ! file_exists( $file ) ) {
			// If fallback is allowed, last resort is default_language without fallback, but do not change the loaded language during fallback:
			return $fallback && static::import( $langPath, 'default_language', $filename, false, $override, false );
		}
		...
		static::addStrings( $strings, $language, $override, $change );
	 ...
	 * @return void
	 */
	public static function addStrings( $array, $language = null, $override = false, $change = true )
	{
		if ( $change ) {
			static::setCurrentLanguage( $language );

			$current	=	static::$self->currentLanguage;
		} elseif ( ( ! $language ) || ( $language === 'default_language' ) ) {
			$current	=	'en-GB';
		} else {
			$current	=	$language;
		}
		...
		if ( ! empty( $array ) ) {
			if ( $override ) {
				if ( isset( static::$self->overrides[$current] ) ) {
					static::$self->overrides[$current]	=	array_merge( static::$self->overrides[$current], $array );
				} else {
					static::$self->overrides[$current]	=	$array;
				}
			} else {
				if ( isset( static::$self->strings[$current] ) ) {
					static::$self->strings[$current]	=	array_merge( static::$self->strings[$current], $array );
				} else {
					static::$self->strings[$current]	=	$array;
				}
			}
		}
	}
